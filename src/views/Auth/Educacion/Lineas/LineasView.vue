...
import { useLineaStore } from '@/stores/lineas'
...
<template>
  <!-- Delete Modal -->
  <DeleteModal
    :modal-text="`La línea de investigación ${store.singleData?.nombre}`"
  ...
  <div class="w-3/4 px-16 pb-8">
    <button @click="create()" class="btn mb-3 ml-4 rounded-lg bg-indigo-700 text-white">
      <font-awesome-icon icon="circle-plus" class="mr-2" />Registrar Línea de Investigación
    </button>
    <!--Table Card-->
    <TableComponent
      title="módulo Líneas de Investigación"
      v-model="searchQuery"
      :columns="columns"
